...
// ✅ سازنده
MybusSession::MybusSession(HttpTransport& transport, const String& deviceId,
...
{
    if (deviceKeypairInitialized_) return true;
    ...
    devicePublicKeyPem_ = "";
    if (!cryptoExportPublicKeyPem(deviceKeypair_, devicePublicKeyPem_)) {
    ...
    if (!cryptoExportPrivateKeyPem(deviceKeypair_, devicePrivateKeyPem_)) {
    ...
    if (devicePublicKeyPem_.isEmpty() || devicePrivateKeyPem_.isEmpty()) {
...
String MybusSession::hmacHex(const uint8_t* key, size_t keyLen,
                              const uint8_t* msg, size_t msgLen)
{
    uint8_t mac[32];
    memset(mac, 0, sizeof(mac));

    if (!hmacSha256(key, keyLen, msg, msgLen, mac)) {
...
{
    hmacHexOut = "";
    if (!sessionKeyValid_ || handshakeNonce_.isEmpty()) {
        Serial.println("[mYBUS] ❌ Cannot create HMAC: session invalid or nonce empty");
        return false;
    }

    hmacHexOut = hmacHex(sessionKey_, sizeof(sessionKey_),
                          reinterpret_cast<const uint8_t*>(handshakeNonce_.c_str()),
...
bool MybusSession::computeSessionKey()
{
    ...
    if (!deviceKeypairInitialized_ || devicePrivateKeyPem_.isEmpty()) {
        Serial.println("[mYBUS] ❌ Device keypair not initialized");
        return false;
    }

    String sharedSecretHex;
    if (!cryptoComputeSharedSecret(devicePrivateKeyPem_, serverPublicKeyPem_, sharedSecretHex) ||
    ...
    static constexpr char kHkdfSalt[] = "mYBUS-v2-Salt";
    static constexpr char kHkdfInfo[] = "mYBUS-v2-Session";

    cryptoSecureZero(sessionKey_, sizeof(sessionKey_));

    bool success = hkdfSha256(
        sharedSecret, sizeof(sharedSecret),
        reinterpret_cast<const uint8_t*>(kHkdfSalt), strlen(kHkdfSalt),
        reinterpret_cast<const uint8_t*>(kHkdfInfo), strlen(kHkdfInfo),
        sessionKey_, sizeof(sessionKey_));
    ...
    // ⚠️ توجه: برخلاف نسخه‌ی قدیمی، اینجا کلید نشست در NVS ذخیره نمی‌شود
    // (به دلایل توضیح‌داده‌شده در پاسخ بالا). بعد از هر ری‌استارت،
    // handshake باید از نو انجام شود.
    return true;
}
...
bool MybusSession::authenticateHandshakeSession(uint32_t requestNumber)
{
    ...
    phase2["communicationInterface"] = interfaceId_;
    phase2["zoneId"] = zone_;
    phase2["deviceId"] = deviceId_;
    phase2["requestNumber"] = requestNumber;
    phase2["flags"] = 0;
    phase2["security"] = mybus_proto::SECURITY_HANDSHAKE;
    phase2["command"] = mybus_proto::COMMAND_HANDSHAKE;

    JsonObject data = phase2["data"].to<JsonObject>();
    data["nonce"] = handshakeNonce_;
    data["hmac"] = hmac;

    String body;
    serializeJson(phase2, body);

    String response = transport_.sendRequest("/devices/handshake", "POST", body, true);
    if (response.isEmpty()) {
    ...
    if (deserializeJson(responseDoc, response) != DeserializationError::Ok) {
        Serial.println("[mYBUS] ❌ Phase 2 JSON parse error");
        return false;
    }
...
// ============================================================
// Full handshake (Phase 1 ECDH -> derive key -> Phase 2 HMAC)
// ============================================================

bool MybusSession::performHandshake(uint32_t requestNumber)
{
    Serial.println("========== mYBUS v2 HANDSHAKE ==========");

    if (!deviceKeypairInitialized_ && !initializeDeviceKeypair()) {
    ...
    phase1["communicationInterface"] = interfaceId_;
    phase1["zoneId"] = zone_;
    phase1["deviceId"] = deviceId_;
    phase1["requestNumber"] = requestNumber;
    phase1["flags"] = 0;
    phase1["security"] = mybus_proto::SECURITY_HANDSHAKE;
    phase1["command"] = mybus_proto::COMMAND_HANDSHAKE;

    JsonObject data = phase1["data"].to<JsonObject>();
    data["publicKeyPem"] = devicePublicKeyPem_;
    data["nonce"] = handshakeNonce_;
    ...
    JsonDocument responseDoc;
    if (deserializeJson(responseDoc, response) != DeserializationError::Ok) {
        Serial.println("[mYBUS] ❌ Phase 1 JSON parse error");
        clear();
        return false;
    }

    const char* serverKey = responseDoc["serverPublicKeyPem"];
    if (serverKey == nullptr || strlen(serverKey) == 0) {
        Serial.println("[mYBUS] ❌ Missing/empty server public key");
        clear();
        return false;
    }

    serverPublicKeyPem_ = String(serverKey);
    ...
    // ---- Phase 2: HMAC ----
    if (!authenticateHandshakeSession(requestNumber)) {
...
// ============================================================
// Clear (does NOT free the device keypair — only session state)
// ============================================================
