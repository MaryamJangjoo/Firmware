...
public:
    MybusTransport(HttpTransport& transport, MybusSession& session);
    ...
    bool sendRegistryFrame(uint16_t regAddr, const uint8_t* regValue,
                            size_t valueLen, bool isWrite,
                            uint8_t busDeviceId, uint32_t requestNumber,
                            JsonDocument* outResponse = nullptr);

    bool sendMybusData(JsonDocument& data, uint32_t requestNumber,
                        JsonDocument* outResponse = nullptr);

private:
    bool decryptAndParseMybusResponse(const uint8_t* wireData, size_t wireLen,
                                       JsonDocument& outDoc);
    void decodeRegistryResponseValue(JsonDocument& doc, uint16_t regAddr);
