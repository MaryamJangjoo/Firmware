...
Serial.printf("[LOCAL-WS] Client %u connected\n",
...
Serial.printf("[LOCAL-WS] Client %u disconnected\n",
static_cast<unsigned>(client->id()));
}
...
Serial.printf("[LOCAL-WS] Client %u auth timeout\n",
...
Serial.println("[LOCAL-WS] Failed to load users.json");
return false;
}
...
if (ok) {
Serial.printf("[LOCAL-WS] Auth OK for '%s'\n", user.c_str());
} else {
Serial.printf("[LOCAL-WS] Wrong password for '%s'\n", user.c_str());
}
return ok;
}
}

Serial.printf("[LOCAL-WS] Unknown user '%s'\n", user.c_str());
return false;
}
...
const auto it = clients_.find(client->id());
if (it == clients_.end()) {
Serial.printf("[LOCAL-WS] Unknown client %u\n",
static_cast<unsigned>(client->id()));
...
Serial.printf("[LOCAL-WS] Rate limit exceeded for client %u\n",
static_cast<unsigned>(client->id()));
...
Serial.printf("[LOCAL-WS] Client %u not authenticated\n",
static_cast<unsigned>(client->id()));
